...
const startSession = async (req, res) => {
  try {
    ...
    const { title, notes, pomodoroSettings } = req.body;
    ...
    if (course.archived) return res.status(400).json({ success: false, message: "Cannot start a session for an archived course" });
...
const getSessions = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ auth0Id: req.auth.payload.sub }).select("courses");
    if (!user) return res.status(404).json({ success: false, message: "User not found" });
